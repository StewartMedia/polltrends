...
def load_spikes(snapshot_date: str | None = None, subdir: str | None = None) -> list[dict]:
    """Load spike annotations from processed data."""
    if snapshot_date:
        target = PROCESSED_DIR / snapshot_date
        if subdir:
...
def build_interest_chart(
    ...
    if spikes:
        for spike in spikes:
            ...
            safe_name = html.escape(entities[party_code]["short_name"])
            safe_label = html.escape(short_label)
            ...
            annotations.append(dict(
                ...
                xref="x",
                yref="y",
                text=f"<b>{safe_name}</b><br>{safe_label}",
                showarrow=True,
                arrowhead=2,
                arrowsize=1,
                arrowwidth=1.5,
...
def build_related_queries_table(
    ...
    for code, ent in entities.items():
        ...
        html_output += f'<div class="rq-card" style="border-left: 4px solid {party_colors[code]}">\n'
        html_output += f'<h3>{ent["short_name"]} — Related Queries</h3>\n'
